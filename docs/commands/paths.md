...
| `--format` | `human\|json\|sarif\|dot\|mermaid\|d2` | `human` | Output format. `dot`, `mermaid`, and `d2` render path-shaped output correctly |
| `--at` | git ref | working HEAD | Pin the query to a historical graph snapshot (e.g. `HEAD~1`, a tag, a commit SHA) |
| `--depth` | integer | `6` | Maximum traversal depth. `0` = unlimited (work-budgeted) |
| `--confidence` | `possible\|probable\|certain` | `possible` | Minimum confidence floor; edges below this tier are excluded |
...
The JSON envelope includes `truncated` and `vacuous` fields for CI consumers; `crosses_exceptional` and `min_confidence` are per-path properties in the `results` array.
...
Paths whose minimum edge confidence falls below `certain` are excluded. Use this when you want to rule out speculative resolution paths.
...
| `2` | Symbol not matched, malformed argument, or bad git ref |
