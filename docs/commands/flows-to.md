...
**Default depth:** The traversal applies a default depth of 2. Pass `--depth 0` for unlimited traversal (work-budgeted; may show `[truncated]` on large graphs).
...
| `--format` | `human\|json\|sarif\|dot\|mermaid\|d2` | `human` | Output format. `dot`, `mermaid`, and `d2` are listed in help but are path-shaped renderers; forest results from `flows-to` may render empty or malformed in those formats. |
...
```
cgx search flow_example --repo /path/to/rust-sample
```
...
`--depth 1` returns only direct downstream nodes, stopping before `b#2`.
...
| `2` | Bad input: no symbol matched the pattern. Also returned when the index was built with `--no-dataflow` and the value-node FQN is absent. |
