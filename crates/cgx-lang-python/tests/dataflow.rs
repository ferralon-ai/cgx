//! Cycle-5 intraprocedural SSA dataflow coverage for the Python adapter.
//!
//! Mirrors Go's `dataflow` test: each production site (assignment / augmented /
//! projection / call / return) lowers into a `DataFlowFact` whose `DerivesFrom`
//! direction is **derived → source** (`y#1 → x#0`). A call result is opaque; a
//! depth-2+ field access truncates to the base local.
...
fn augmented_assignment_derives_from_self_and_operand() {
    ...
    let f = flows("def g(x, y):\n    x += y\n    return x\n");
    assert!(has_flow(&f, "x", "x", Transform::Arith), "self-dep: {f:?}");
    assert!(has_flow(&f, "x", "y", Transform::Arith), "operand-dep: {f:?}");
    // The augmented write is SSA version 1 (params are #0 roots).
    let aug = f
        .iter()
...
#[test]
fn attribute_projection_binds_to_base_local() {
    // Depth-1 `a.field` binds to the BASE local `a`, NOT the field name (v0.3 Gap 2).
    let f = flows("def g(a):\n    b = a.field\n    return b\n");
    let proj = f
        .iter()
    ...
    assert!(
        ...
        "depth-1 is not truncated: {proj:?}"
    );
}

#[test]
fn subscript_projection_binds_to_base_local() {
    let f = flows("def g(a, i):\n    b = a[i]\n    return b\n");
    let proj = f
        .iter()
        .find(|x| x.derived.last().map(String::as_str) == Some("b"))
        .expect("projection fact");
    assert_eq!(proj.transform, Transform::Projection);
    assert_eq!(proj.source.first().map(String::as_str), Some("a"), "{proj:?}");
}
...
fn deep_field_truncates_access_path() {
    ...
    // Truncated to the base identifier `a`.
...
#[test]
fn tuple_unpack_each_target_derives_from_rhs() {
    // `p, q = f()`: best-effort — each LHS derives from the opaque call result.
    let f = flows("def g():\n    p, q = f()\n    return p\n");
    let p = f
        .iter()
...
#[test]
fn walrus_binds_and_derives() {
    // `(v := a)` binds `v` deriving (copy) from `a`.
    let f = flows("def g(a):\n    if (v := a):\n        return v\n    return a\n");
    assert!(has_flow(&f, "v", "a", Transform::Copy), "got {f:?}");
}

#[test]
fn list_literal_is_composed_from_elements() {
    let f = flows("def g(a, b):\n    p = [a, b]\n    return p\n");
...
fn no_phantom_param_source_facts() {
    // Parameters are SSA roots: they must never appear as a `derived` of a fact
    // (no dead phantom `#0` write — v0.3 Gap A).
    let f = flows("def g(a, b):\n    c = a + b\n    return c\n");
    assert!(
        !f.iter().any(|x| {
            matches!(x.derived.last().map(String::as_str), Some("a") | Some("b"))
        }),
