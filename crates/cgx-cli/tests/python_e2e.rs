//! Python language-coverage end-to-end smoke: prove the language-agnostic
//! index/query/dataflow pipeline lights up on the committed `fixtures/python`
//! tree via the *built `cgx` binary*, mirroring the `go_e2e` harness.
//!
//! Claim: `flows-to`/`flows-from` resolve the intraprocedural chain
//! a -> b -> c -> return in `dataflow.py`, proving the `DerivesFrom` edge
//! DIRECTION (derived -> source) survives the real resolver end-to-end. (The
//! per-transform-class coverage lives in the `cgx-lang-python` `dataflow` suite.)
//!
//! Value-node FQNs (Python module prefix = file stem; `cgx search`):
//!   dataflow::transform::a#0       (the parameter, SSA root)
//!   dataflow::transform::b#1       (b = a)
//!   dataflow::transform::c#1       (c = b + 1)
//!   dataflow::transform::return#1  (return c)
...
const A: &str = "dataflow::transform::a#0";
const B: &str = "dataflow::transform::b#1";
const C: &str = "dataflow::transform::c#1";
const RETURN: &str = "dataflow::transform::return#1";

/// Absolute path to the workspace `fixtures/python` source tree.
...
/// Copy `fixtures/python` into a fresh temp dir, `git init` it, and commit so it
/// has a real HEAD tree OID. Returns the temp dir (kept alive by the caller).
...
fn index(repo: &Path) {
    ...
    assert_eq!(code, 0, "index of fixtures/python should succeed: {e}");
}
...
fn flows_to_resolves_transform_chain() {
    // `flows-to a` walks the DerivesFrom forward slice a -> b -> c -> return in
    // `dataflow.py`. Because DerivesFrom points derived -> source, `flows-to a`
    // (reverse walk) reaching b/c/return proves the edges were emitted in the
    // correct direction by the Python frontend.
...
fn flows_from_return_walks_pedigree_to_param() {
    ...
    // Reaching `a` proves the chain is connected in the source direction.
    let (_tmp, repo) = fixture_repo();
    index(&repo);
