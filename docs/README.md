...
are deterministic and reproducible. The core differentiating capability is
exception-path edge labeling — every call edge is annotated `always`, `conditional`,
`exception`, `loop`, or `panic`, enabling queries that no existing tool serves (OWASP A10:2025
validates the need). Graph diff across commits and branches is the second major novel
capability.
...
| 02 | [02-personas-and-questions.md](02-personas-and-questions.md) | Persona-driven question inventory: 85 questions across 9 themes, 67 NOVEL; four personas (PSE, SSE, ACA, ASA); all four canonical prompt examples |
...
| 10 | [10-landscape.md](10-landscape.md) | Comparison with CodeQL, Joern, Sourcegraph/SCIP, Glean, Kythe, Semgrep, rust-analyzer, stack-graphs, Rupta, Serena, codegraph variants; explicit gaps `cgx` fills |
| 11 | [11-roadmap.md](11-roadmap.md) | Phasing: MVP → semantic precision → dataflow → branch/VCS features → MCP polish; risks and open questions |
...
**Security engineer:** Read 01, then 02 (all PSE questions and themes 1, 3, 4, 6, 9).
